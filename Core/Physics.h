...
struct PhysicalObject
{
	AABB borders;
	char type;
	bool operator==(PhysicalObject obj)
	{
		return this->type == obj.type && this->borders.max.x == obj.borders.max.x && this->borders.max.y == obj.borders.max.y && this->borders.min.x == obj.borders.min.x && this->borders.min.y == obj.borders.min.y;
	}
};
...
// Checks collision of a and b
inline bool NormalAABBvsAABB(AABB a, AABB b)
{
	// Exit with no intersection if found separated along an axis
	if (a.max.x < b.min.x or a.min.x > b.max.x) return false;
	if (a.max.y < b.min.y or a.min.y > b.max.y) return false;
